...
#ifndef _dct_col_inbuf_H_
#define _dct_col_inbuf_H_

#include <systemc>
#include <iostream>
#include <fstream>

#include "dct_col_inbuf_memcore.h"

SC_MODULE(dct_col_inbuf) {
    static const unsigned int DataWidth    = 128;
...
protected:
    ...
    dct_col_inbuf_memcore* dct_col_inbuf_memcore_U;
...
public:
    ~dct_col_inbuf();
    SC_CTOR(dct_col_inbuf) : m_trace_file(0) {
        SC_METHOD(proc_memcore_addr);
        sensitive << i_address0 << t_address0 << iptr << tptr;

        dct_col_inbuf_memcore_U = new dct_col_inbuf_memcore("dct_col_inbuf_memcore_U");
        dct_col_inbuf_memcore_U->clk      ( clk );
        dct_col_inbuf_memcore_U->reset    ( reset );
        dct_col_inbuf_memcore_U->ce0      ( i_ce0 );
        dct_col_inbuf_memcore_U->we0      ( i_we0 );
        dct_col_inbuf_memcore_U->address0 ( memcore_iaddr );
        dct_col_inbuf_memcore_U->d0       ( i_d0 );
        dct_col_inbuf_memcore_U->q0       ( i_q0 );

        dct_col_inbuf_memcore_U->ce1      ( t_ce0 );
        dct_col_inbuf_memcore_U->we1      ( t_we0 );
        dct_col_inbuf_memcore_U->address1 ( memcore_taddr );
        dct_col_inbuf_memcore_U->d1       ( t_d0 );
        dct_col_inbuf_memcore_U->q1       ( t_q0 );
        ...
        if (dump_vcd) {
            m_trace_file = sc_create_vcd_trace_file( "dct_col_inbuf.trace");
            // ports
